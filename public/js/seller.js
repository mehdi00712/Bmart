...
// 🔧 Cloudinary (unsigned) config (provided cloud_name). Create an unsigned preset in your Cloudinary console.
const CLOUD_NAME = 'degcslkrj';
const UPLOAD_PRESET = 'unsigned_marketplace'; // change to your actual unsigned preset name if different
...
pform.addEventListener('submit', async (e) => {
  ...
  const prod = {
    ...
    ownerUid: user.uid,
    shopId: user.uid, // simple: one shop per seller
    isActive: true,
...
async function loadProducts() {
  ...
  plist.innerHTML = items.map(p => `
    <div class="row">
      ...
      <div>
        <b>${p.title}</b><br>${fmtCurrency(p.price)} · Stock: ${p.stock}
      </div>
  ...
  ordersDiv.innerHTML = orders.map(o => `
    <div class="card">
      ...
      <select data-id="${o.id}" class="status">
        ${['pending','accepted','out_for_delivery','completed','cancelled'].map(s => `<option ${s===o.status?'selected':''}>${s}</option>`).join('')}
      </select>
    </div>`).join('');
  $$('.status', ordersDiv).forEach(sel => sel.onchange = async (e) => {
