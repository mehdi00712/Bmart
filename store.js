...
export function removeFromCart(productId) { setCart(getCart().filter(x => x.productId !== productId)); }
export function clearCart() { setCart([]); }
export function subtotal() { return getCart().reduce((s, it) => s + (it.price * it.qty), 0); }
